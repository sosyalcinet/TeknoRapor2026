...
# --- 1. GÜVENLİK VE YENİ ANAHTAR ---
# Sizin paylaştığınız en güncel anahtar: AIzaSyBaIxXeOl4ZjDscnRotVB0PReXQCz9uFd8
GEMINI_API_KEY = "AIzaSyBaIxXeOl4ZjDscnRotVB0PReXQCz9uFd8"
...
st.set_page_config(page_title="TeknoRapor V23 | Derepazarı", layout="centered", page_icon="🤖")

# --- 3. FONKSİYONLAR (Çalışan Koddan Alındı) ---
...
def create_word_official(text, info):
    doc = Document()
    doc.add_heading(info['y_adi'], 0).alignment = WD_ALIGN_PARAGRAPH.CENTER
...
# --- 4. ARAYÜZ TASARIMI ---
st.markdown("<h1 style='text-align: center; color: #1E3A8A;'>Teknofest Resmi ÖDR Robotu V23</h1>", unsafe_allow_html=True)
...
# PROJE ADI VE ÖZETİ - İSTEDİĞİNİZ GİBİ BİR ARADA (ALT ALTA)
with st.expander("📝 Proje Detayı ve Yazım Ayarları", expanded=True):
    ...
    yazim_modu = st.selectbox("Yazım Karakteri", ["Standart AI Dedektör (İnsan Gibi Yaz)", "Akademik", "Süper AI"])
    ...
    if st.button("🚀 Şartnameye Uygun Raporu Hazırla", use_container_width=True, type="primary"):
        if not aciklama or not p_adi_input:
            ...
            with st.status("🛠️ Rapor hazırlanıyor...", expanded=True) as status:
                try:
                    # ÇALIŞAN KODUN MODEL ÇAĞIRMA MANTIĞI
                    ...
                    Hedef: {hedef_sayfa} sayfa. Tema: {ana_t}/{alt_t}.
                    ---
